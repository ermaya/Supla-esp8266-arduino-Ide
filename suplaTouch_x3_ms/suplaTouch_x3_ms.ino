...
void suplaDigitalWrite(int channelNumber, uint8_t pin, uint8_t val) {       
     ...
     if ((monostable == false) && (CustomSupla.channel_pin[channelNumber].DurationMS != btn[channelNumber].ms)) {
            int durationMS = CustomSupla.channel_pin[channelNumber].DurationMS;
               ...
               EEPROM.put((channelNumber * 10) + 100, durationMS);
               EEPROM.commit();
...
void setup() {  //------------------------------------------------ Setup ----------------------------------------------
  ...
  btn[0].ms =0;EEPROM.get(100,btn[0].ms);Serial.print("initial_button_duration 0: ");Serial.println(btn[0].ms); 
  btn[0].mem =0;
  btn[1].pin =button_2 +1;          // pin gpio buton  +1
  btn[1].relay_pin =relay_2 +1;  // pin gpio Relay   +1
  btn[1].channel =1;      // channel
  btn[1].ms =0;EEPROM.get(110,btn[1].ms);Serial.print("initial_button_duration 1: ");Serial.println(btn[1].ms);
  btn[1].mem =0;
  btn[2].pin =button_3 +1;          // pin gpio buton  +1
  btn[2].relay_pin =relay_3 +1;  // pin gpio Relay   +1
  btn[2].channel =2;      // channel
  btn[2].ms =0;EEPROM.get(120,btn[2].ms);Serial.print("initial_button_duration 2: ");Serial.println(btn[2].ms);
  btn[2].mem =0;
  supla_btn_init();
  ...
  CustomSupla.setTimerFuncImpl(&supla_timer);
