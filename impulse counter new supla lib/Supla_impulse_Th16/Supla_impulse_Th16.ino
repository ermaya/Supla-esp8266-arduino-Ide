...
void setup() {
  ...
  if (SPIFFS.begin()) {
   ...
   char AUTHKEY[SUPLA_AUTHKEY_SIZE] = {mac[WL_MAC_ADDR_LENGTH - 1], mac[WL_MAC_ADDR_LENGTH - 2],mac[WL_MAC_ADDR_LENGTH - 3], mac[WL_MAC_ADDR_LENGTH - 4],
    ...
    SuplaDevice.addImpulseCounter(14, 0, true, true, 10);  // int _impulsePin, int _statusLedPin, bool _detectLowToHigh, bool _inputPullup, unsigned long _debounceDelay
    SuplaDevice.addRelay(12, false);           // ? - ?Pin number where the relay is connected  
