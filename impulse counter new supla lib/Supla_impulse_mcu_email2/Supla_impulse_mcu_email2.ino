...
Supla::ESPWifi wifi("", "");  //------ Do not change----
...
void setup() {
  ...
  {
    Serial.print("local IP: ");Serial.println(WiFi.localIP());Serial.print("subnetMask: ");Serial.println(WiFi.subnetMask());Serial.print("gatewayIP: ");Serial.println(WiFi.gatewayIP());long rssi = WiFi.RSSI();Serial.print("Signal Strength (RSSI): ");Serial.print(rssi);Serial.println(" dBm");
  });
  ...
  if (SPIFFS.begin()) {
   ...
   char AUTHKEY[SUPLA_AUTHKEY_SIZE] = {mac[WL_MAC_ADDR_LENGTH - 1], mac[WL_MAC_ADDR_LENGTH - 2],mac[WL_MAC_ADDR_LENGTH - 3], mac[WL_MAC_ADDR_LENGTH - 4],
    ...
    SuplaDevice.addImpulseCounter(5, 0, true, true, 10);  // int _impulsePin, int _statusLedPin, bool _detectLowToHigh, bool _inputPullup, unsigned long _debounceDelay
    SuplaDevice.addRelay(2, false);           // ? - ?Pin number where the relay is connected  
