...
void get_temperature_and_humidity(int channelNumber, double *temp, double *humidity) {
    ...
    float v = pzem.voltage();    // ------------------- We get the voltage.----------
    if (v < 0.0) v = 0.0;
    Serial.print(v);Serial.print("V; ");
    *temp = v;  // --------------------------------------voltage as temperature ------------
    float p = pzem.power(); // ------------------------- We get power. -----------
    if (p < 0.0) p = 0.0;
    Serial.print(p);Serial.print("W; ");
    *humidity = p / 1000;// --------we reduce to Kw, to be able to see the humidity temperature charts. If we send in watts we will see in the App but there will be no charts.
    float e = pzem.energy();  // ---------we get energy ------ we take advantage of the temperature and humidity reading cycle every 10 seconds for this. -------
    if(e >= 0.0){ Serial.print(e);Serial.print("Wh; ");
     if ( e != energ){ // ----------------- We check if the value has changed. If so, we send the updated value. ---------
      energ = e;
      SuplaImpulseCounter::getCounterByChannel(0)->setCounter(energ); // ------ function to adjust the counter value. -----------
     }    
    }
