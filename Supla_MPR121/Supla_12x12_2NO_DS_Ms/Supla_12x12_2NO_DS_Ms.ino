  ...
  public:
    void customDigitalWrite(int channelNumber, uint8_t pin, uint8_t val) {
      if ((pin >= 100) && (pin <= 115)){
        if (sensor){
          ...
          Serial.println("off by sensor (Write)");
        } else {
   ...
   int customDigitalRead(int channelNumber, uint8_t pin) {
      ...
      if (pin <= 99){
        ...
        if ((pin == sensor_pin ) && (sensor)){
          Wire.beginTransmission(0x20);
          Wire.write(0x12); // address port A
          Wire.write(0);  // value to send
          Wire.endTransmission();
          Serial.println("off by sensor (Read)");
        }          
      }
    }   
}MyMcp23017;
...
void ondemandwifiCallback () {
  ...
  // wifiManager.setCustomHeadElement("<style>html{ background-color: #01DF3A;}</style><div class='s'><svg version='1.1' id='l' x='0' y='0' viewBox='0 0 200 200' xml:space='preserve'><path d='M59.3,2.5c18.1,0.6,31.8,8,40.2,23.5c3.1,5.7,4.3,11.9,4.1,18.3c-0.1,3.6-0.7,7.1-1.9,10.6c-0.2,0.7-0.1,1.1,0.6,1.5c12.8,7.7,25.5,15.4,38.3,23c2.9,1.7,5.8,3.4,8.7,5.3c1,0.6,1.6,0.6,2.5-0.1c4.5-3.6,9.8-5.3,15.7-5.4c12.5-0.1,22.9,7.9,25.2,19c1.9,9.2-2.9,19.2-11.8,23.9c-8.4,4.5-16.9,4.5-25.5,0.2c-0.7-0.3-1-0.2-1.5,0.3c-4.8,4.9-9.7,9.8-14.5,14.6c-5.3,5.3-10.6,10.7-15.9,16c-1.8,1.8-3.6,3.7-5.4,5.4c-0.7,0.6-0.6,1,0,1.6c3.6,3.4,5.8,7.5,6.2,12.2c0.7,7.7-2.2,14-8.8,18.5c-12.3,8.6-30.3,3.5-35-10.4c-2.8-8.4,0.6-17.7,8.6-22.8c0.9-0.6,1.1-1,0.8-2c-2-6.2-4.4-12.4-6.6-18.6c-6.3-17.6-12.7-35.1-19-52.7c-0.2-0.7-0.5-1-1.4-0.9c-12.5,0.7-23.6-2.6-33-10.4c-8-6.6-12.9-15-14.2-25c-1.5-11.5,1.7-21.9,9.6-30.7C32.5,8.9,42.2,4.2,53.7,2.7c0.7-0.1,1.5-0.2,2.2-0.2C57,2.4,58.2,2.5,59.3,2.5z M76.5,81c0,0.1,0.1,0.3,0.1,0.6c1.6,6.3,3.2,12.6,4.7,18.9c4.5,17.7,8.9,35.5,13.3,53.2c0.2,0.9,0.6,1.1,1.6,0.9c5.4-1.2,10.7-0.8,15.7,1.6c0.8,0.4,1.2,0.3,1.7-0.4c11.2-12.9,22.5-25.7,33.4-38.7c0.5-0.6,0.4-1,0-1.6c-5.6-7.9-6.1-16.1-1.3-24.5c0.5-0.8,0.3-1.1-0.5-1.6c-9.1-4.7-18.1-9.3-27.2-14c-6.8-3.5-13.5-7-20.3-10.5c-0.7-0.4-1.1-0.3-1.6,0.4c-1.3,1.8-2.7,3.5-4.3,5.1c-4.2,4.2-9.1,7.4-14.7,9.7C76.9,80.3,76.4,80.3,76.5,81z M89,42.6c0.1-2.5-0.4-5.4-1.5-8.1C83,23.1,74.2,16.9,61.7,15.8c-10-0.9-18.6,2.4-25.3,9.7c-8.4,9-9.3,22.4-2.2,32.4c6.8,9.6,19.1,14.2,31.4,11.9C79.2,67.1,89,55.9,89,42.6z M102.1,188.6c0.6,0.1,1.5-0.1,2.4-0.2c9.5-1.4,15.3-10.9,11.6-19.2c-2.6-5.9-9.4-9.6-16.8-8.6c-8.3,1.2-14.1,8.9-12.4,16.6C88.2,183.9,94.4,188.6,102.1,188.6z M167.7,88.5c-1,0-2.1,0.1-3.1,0.3c-9,1.7-14.2,10.6-10.8,18.6c2.9,6.8,11.4,10.3,19,7.8c7.1-2.3,11.1-9.1,9.6-15.9C180.9,93,174.8,88.5,167.7,88.5z'/></svg>");
   ...
   if (!wifiManager.startConfigPortal("Supla_12X12")) { Serial.println("Not connected to WiFi but continuing anyway.");} else { Serial.println("connected...yeey :)");}                
...
void guid_authkey(void) {
  if (EEPROM.read(300) != 60){
    ...
    EEPROM.put(600, 0); EEPROM.put(610, 0); EEPROM.put(620, 0); EEPROM.put(630, 0); EEPROM.put(640, 0); EEPROM.put(650, 0); 
    EEPROM.put(660, 0); EEPROM.put(670, 0); EEPROM.put(680, 0); EEPROM.put(690, 0); EEPROM.put(700, 0); EEPROM.put(710, 0);
    EEPROM.write(300, 60);
    EEPROM.commit();
    delay(0);
  }
...
void setup() {
  ...
  btn[0].pin =1; EEPROM.get(600,btn[0].ms);Serial.print("initial_button_duration 0: ");Serial.println(btn[0].ms);          
  btn[1].pin =2; EEPROM.get(610,btn[1].ms);Serial.print("initial_button_duration 1: ");Serial.println(btn[1].ms);    
  btn[2].pin =3; EEPROM.get(620,btn[2].ms);Serial.print("initial_button_duration 2: ");Serial.println(btn[2].ms);    
  btn[3].pin =4; EEPROM.get(630,btn[3].ms);Serial.print("initial_button_duration 3: ");Serial.println(btn[3].ms);    
  btn[4].pin =5; EEPROM.get(640,btn[4].ms);Serial.print("initial_button_duration 4: ");Serial.println(btn[4].ms);    
  btn[5].pin =6; EEPROM.get(650,btn[5].ms);Serial.print("initial_button_duration 5: ");Serial.println(btn[5].ms);    
  btn[6].pin =7; EEPROM.get(660,btn[6].ms);Serial.print("initial_button_duration 6: ");Serial.println(btn[6].ms);    
  btn[7].pin =8; EEPROM.get(670,btn[7].ms);Serial.print("initial_button_duration 7: ");Serial.println(btn[7].ms);    
  btn[8].pin =9; EEPROM.get(680,btn[8].ms);Serial.print("initial_button_duration 8: ");Serial.println(btn[8].ms);  
  btn[9].pin =10; EEPROM.get(690,btn[9].ms);Serial.print("initial_button_duration 9: ");Serial.println(btn[9].ms);  
  btn[10].pin =11; EEPROM.get(700,btn[10].ms);Serial.print("initial_button_duration 10: ");Serial.println(btn[10].ms);  
  btn[11].pin =12; EEPROM.get(710,btn[11].ms);Serial.print("initial_button_duration 11: ");Serial.println(btn[11].ms);  
    supla_btn_init();
  ...
  if (SPIFFS.begin()) {  // ------------------------- wificonfig read -----------------
   ...
   WiFi.mode(WIFI_STA); 
      ...
      new Supla::Sensor::NormallyOpen(sensor_pin, true);
      new Supla::Sensor::NormallyOpen(sensor2_pin, true);
      new Supla::Sensor::DS18B20(DS18B20_pin);
  ...
  SuplaDevice.iterate();
   ...
   switch (s) { 
    case 17:      // -----     STATUS_REGISTERED_AND_READY
     if (Tik == true){ticker.detach();Tik = false;digitalWrite(onboard_led, HIGH);}
     if (state10 == true){
      for (int i = 0; i < 16; i++) {
        if (btn[i].ms<10){
...
void MPR121Relay(){
   ...
   if ( btn[an].ms > 0 ) {
                     Serial.print("monostable time: ");Serial.println(btn[an].ms);
                     monostable = true;
                 if ( mcp.digitalRead(an) == 1 ) {  
                  SuplaDevice.relayOff(an);
                  Serial.print("BTN Switsh off relay: ");
                  Serial.println(an);
                 } else {                     
                     SuplaDevice.relayOn(an, btn[an].ms);}
                 } else {
                 if ( mcp.digitalRead(an) == 1 ) {  
                  SuplaDevice.relayOff(an);
                  Serial.print("BTN Switsh off relay: ");
