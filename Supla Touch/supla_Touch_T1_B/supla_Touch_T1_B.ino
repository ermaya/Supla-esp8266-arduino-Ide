...
void iterate_botton() {
  ...
  for(int a=0;a<BTN_COUNT;a++)
    if (btn[a].pin > 0) {
        ...
        if (v != btn[a].last_val){btn[a].last_time = now;}
        if ( now - btn[a].last_time > debon ){
         if (v != btn[a].act_state) {     
          btn[a].act_state = v; 
           if (v==0)
             {
              booton_action = true;
...
void supla_btn_init() {
  for(int a=0;a<BTN_COUNT;a++)
    if (btn[a].pin > 0) {
        ...
        btn[a].act_state = btn[a].last_val;
...
void suplaDigitalWrite(int channelNumber, uint8_t pin, uint8_t val) {       
     ...
     if ((booton_action == false) && (CustomSupla.channel_pin[channelNumber].DurationMS != btn[channelNumber].ms) && (!val)) {
...
void setup() {  //------------------------------------------------ Setup ----------------------------------------------
  ...
  CustomSupla.begin(GUID, mac, Supla_server, LocationID, Location_Pass);
   btn_loop.attach_ms(50, Btn_loop);                     
}
