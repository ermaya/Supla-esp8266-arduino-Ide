  ...
  if (pin < 16) {
    if (mode == OUTPUT) {
      ...
      if (mode == INPUT_PULLUP) {
       ...
       // updateReg(pin < 8 ? MCP23017_IPOLA : MCP23017_IPOLB, 0xFF, 1 << (pin % 8)); // 1 = GPIO register bit will reflect the opposite logic state of the input pin.
      } else {
        updateReg(pin < 8 ? MCP23017_GPPUA : MCP23017_GPPUB, ~(uint8_t)(1 << (pin % 8)), 0x00);
       // updateReg(pin < 8 ? MCP23017_IPOLA : MCP23017_IPOLB, ~(uint8_t)(1 << (pin % 8)), 0x00); // 0 = GPIO register bit will reflect the same logic state of the input pin.
      }
    }
  }
}
