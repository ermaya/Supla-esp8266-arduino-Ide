...
int relay_4 = 14;        //------------------- relay output ----- active high
...
#include "StatusLed.h"      //for LED status--Dependent of EasyLed-----https://github.com/evert-arias/EasyLed
...
void ondemandwifiCallback () {
        ...
        StatusLedOn();  //---------led on when wificonfig is opened (no callback her)
  ...
  WiFiManagerParameter custom_up_host("DHCP_name", "xxxx (DHCP name)", up_host, 21,"required");
...
void saveParamCallback(){
  ...
  Serial.println("PARAM radio_b = " + getParam("radio_b"));
...
void Eepron_read() {                  //----------EEPROM read  ---------------------- EEprom  
       ...
       if ( btn[epr].ms > 0 ) {
         ...
         t = sensors.getTempCByIndex(0);
         if (t < -100){t = -275;} 
                  SuplaDevice.channelDoubleValueChanged(3,t);         
         double trd = (Term_Temp) ;
                  trd = (trd / 10) ; 
                  SuplaDevice.channelDoubleValueChanged(4,trd);
...
void loop() {
  ...
  int C_W_read = digitalRead(btn[0].pin-1);{  
   ...
   StatusLed(s);  //------------------------------------------------ StatusLed callback --------------------- 
