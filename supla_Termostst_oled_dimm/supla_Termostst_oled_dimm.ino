...
#include "SSD1306Wire.h" // legacy include: `#include "SSD1306.h"`  display.drawString(0, 15, "H: " + String(temperature0ValueToDisplay) + "/" + String(temperature1ValueToDisplay) + " °C");
...
int relay_1 = 15;        //D8 ------------------- thermometer on off Led ----- active high
...
int C_n_state = HIGH; 
int last_C_n_state = HIGH;
unsigned long time_last_C_n_change = 0; 
int C_p_state = HIGH; 
int last_C_p_state = HIGH;
unsigned long time_last_C_p_change = 0; 
long C_delay = 100; 
bool tikOn = false;
WiFiClient client;
...
void TH_Overlay() {
  ...
  display.clear();
   ...
   if ((btn[0].mem) == 1){
         display.setFont(ArialMT_Plain_16);
         display.setTextAlignment(TEXT_ALIGN_CENTER);
         display.drawString(64, 48, "Set " + String(trs, 2) + "ºC"); 
  }
  ...
  if ((btn[1].mem) == 0){
         display.setFont(ArialMT_Plain_16);
         display.setTextAlignment(TEXT_ALIGN_CENTER);
         display.drawString(90, 0, "Relay OFF");  
  }
  if ((btn[1].mem) == 1){
         display.setFont(ArialMT_Plain_16);
         display.setTextAlignment(TEXT_ALIGN_CENTER);
         display.drawString(90, 0, "Relay ON");
  }
  display.display();
  yield();  
}
