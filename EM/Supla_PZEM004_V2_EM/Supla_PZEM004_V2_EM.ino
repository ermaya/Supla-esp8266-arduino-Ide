...
class PZEMv2 : public Supla::OnePhaseElectricityMeter {
    public:
...
void ondemandwifiCallback () {
   ...
   if (!wifiManager.startConfigPortal("PZEM_v2")) { Serial.println("Not connected to WiFi but continuing anyway.");} else { Serial.println("connected...yeey :)");}                
...
void setup() {
  ...
  if (SPIFFS.begin()) {  // ------------------------- wificonfig read -----------------
   ...
   char AUTHKEY[SUPLA_AUTHKEY_SIZE] = {mac[WL_MAC_ADDR_LENGTH - 1], mac[WL_MAC_ADDR_LENGTH - 2],mac[WL_MAC_ADDR_LENGTH - 3], mac[WL_MAC_ADDR_LENGTH - 4],
    ...
    new PZEMv2(5, 4);  // (RX,TX) connect to TX,RX of PZEM 
