...
class MyCSE7766 : public Supla::Sensor::OnePhaseElectricityMeter {
    public:
        ...
        virtual void readValuesFromDevice() {
          myCSE7766.handle(); 
          mem_energy = _mem_energy + (myCSE7766.getEnergy() / 36 );    // mem_energy = 0.01 Wh                              
            // voltage in 0.01 V
...
void ondemandwifiCallback () {
   ...
   WiFiManagerParameter custom_Supla_name("name", "Supla Device Name", Supla_name, 50,"required");
   ...
   if (!wifiManager.startConfigPortal("GosundP1")) { Serial.println("Not connected to WiFi but continuing anyway.");} else { Serial.println("connected...yeey :)");}                
...
void loop() {
  ...
  if (relay4->isOn()){
   ...
   if (((s != 17) || (WiFi.status() != WL_CONNECTED)) && (Tik == false)) { ticker.attach(0.5, tick);Tik = true; } 
 
   if(millis() > mem_energy_milis){
    if ((mem_energy > _mem_energy) && ((relay1->isOn()) || (relay2->isOn()) || (relay3->isOn()))){
      energy_save();
     mem_energy_milis = millis() + 600000 ;  
    }
   }  
...
void longClickA(Button3& btn) {
  if ((relay1->isOn()) || (relay2->isOn()) || (relay3->isOn()) || (relay4->isOn())){
    relay1->turnOff();relay2->turnOff();relay3->turnOff();relay4->turnOff();
  } else {
    relay1->turnOn();relay2->turnOn();relay3->turnOn();relay4->turnOn();
  }
}
