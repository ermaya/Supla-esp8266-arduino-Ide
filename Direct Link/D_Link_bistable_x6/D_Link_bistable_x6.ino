...
char D_Link_6[40];
byte mac[6];
String url = "/direct/xx/xxxxxxxxx/toggle";
...
bool last_5 = false;
bool last_6 = false;
#define Config_PIN D0     // Triger Wifi config 
...
void ondemandwifiCallback () {
  ...
  WiFiManagerParameter custom_Supla_server("server", "svrXX.supla.org", Supla_server, 20);
  WiFiManagerParameter custom_D_Link_1("Link 1", "/direct/xx/xxxxxxxxx/toggle", D_Link_1, 40);
  ...
  wifiManager.setMinimumSignalQuality(10);
    ...
    if (!wifiManager.startConfigPortal("DirectLink6B")) {
...
void setup() {  //------------------------------------------------ Setup ----------------------------------------------
  ...
  last_2 = digitalRead(Link_2_PIN);
  last_3 = digitalRead(Link_3_PIN);
  last_4 = digitalRead(Link_4_PIN);
  last_5 = digitalRead(Link_5_PIN);
  last_6 = digitalRead(Link_6_PIN);
...
void loop() { 
 ...
 if ( digitalRead(Link_1_PIN) != last_1 )  {
   ...
   if ( digitalRead(Link_2_PIN)!= last_2)  {
    digitalWrite(onboard_led, LOW);
    url = D_Link_2;
    direct_Link() ; 
    last_2 = digitalRead(Link_2_PIN);   
  }
  if ( digitalRead(Link_3_PIN) != last_3 )  {
    digitalWrite(onboard_led, LOW);
    url = D_Link_3;
    direct_Link() ;
    last_3 = digitalRead(Link_3_PIN);   
  }
   if ( digitalRead(Link_4_PIN) != last_4)  {
    digitalWrite(onboard_led, LOW);
    url = D_Link_4;
    direct_Link() ;
    last_4 = digitalRead(Link_4_PIN);    
  }
  if ( digitalRead(Link_5_PIN) != last_5)  {
    digitalWrite(onboard_led, LOW);
    url = D_Link_5;
    direct_Link() ;
    last_5 = digitalRead(Link_5_PIN);    
  }
   if ( digitalRead(Link_6_PIN) != last_6)  {
    digitalWrite(onboard_led, LOW);
    url = D_Link_6;
    direct_Link() ;
    last_6 = digitalRead(Link_6_PIN);    
