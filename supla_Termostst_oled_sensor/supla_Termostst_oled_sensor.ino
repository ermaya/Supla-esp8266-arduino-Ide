...
unsigned long time_last_sensor_change = 0; 
long sensor_delay = 500;           
...
void TH_Overlay() {
  ...
  display.clear();
   ...
   if (window == true){
         display.setFont(ArialMT_Plain_16);
         display.setTextAlignment(TEXT_ALIGN_CENTER);
         display.drawString(64, 48, "STOPED " + String(trs, 1) + "ºC");            
   }else{
        if ((btn[0].mem) == 1){
         display.setFont(ArialMT_Plain_16);
         display.setTextAlignment(TEXT_ALIGN_CENTER);
  ...
  if ((btn[1].mem) == 0){
         display.setFont(ArialMT_Plain_16);
         display.setTextAlignment(TEXT_ALIGN_LEFT);
         display.drawString(92, 0, "OFF");  
  }
  if ((btn[1].mem) == 1){
         display.setFont(ArialMT_Plain_16);
         display.setTextAlignment(TEXT_ALIGN_RIGHT);
         display.drawString(90, 0, "ON");
  }
  display.display();
  yield();  
}
...
yield(); 
  ...
  int C_W_read = digitalRead(btn[0].pin-1);{  
   ...
   int sensor_read = digitalRead(sensor_pin);{  
   if (sensor_read != last_sensor_state) {            
     time_last_sensor_change = millis();
   }
   if ((millis() - time_last_sensor_change) > sensor_delay) {     
     if (sensor_read != sensor_state) {     
       Serial.println("window sate changed");
       sensor_state = sensor_read; 
       window = sensor_state;             
     }
    }
   last_sensor_state = sensor_read;            
   }
