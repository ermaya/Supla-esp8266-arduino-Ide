...
void suplaDigitalWrite(int channelNumber, uint8_t pin, uint8_t val) {       
     btn[channelNumber].mem =val; 
      if ( (btn[channelNumber].mem) == 0 ) {                    
                  ...
                  digitalWrite(btn[channelNumber].relay_pin-1, !LOW);       //-------------  ! for active low        
                  eep_milis = millis() + 2000 ;                  
                 }else {
                  Serial.print("Switsh on relay ");
                  Serial.println(btn[channelNumber].relay_pin-1);
                  digitalWrite(btn[channelNumber].relay_pin-1, !HIGH);       //-------------  ! for active low        
                  eep_milis = millis() + 2000 ;                   
                 }
  return; 
}
...
void setup() {  //------------------------------------------------ Setup ----------------------------------------------
  ...
  btn[0].pin =bt_1 +1;          // pin gpio buton  +1
  btn[0].relay_pin =rl_1 +1;  // pin gpio Relay   +1
  btn[0].channel =0;      // channel
  btn[0].ms =0;           //  if = 0 Bistable -- if > 0 Monostable for X ms
  btn[0].mem =0;
  btn[1].pin =bt_2 +1;          // pin gpio buton  +1
  btn[1].relay_pin =rl_2 +1;  // pin gpio Relay   +1
  btn[1].channel =1;      // channel
  btn[1].ms =0;           //  if = 0 Bistable -- if > 0 Monostable for X ms
  btn[1].mem =0;
  btn[2].pin =bt_3 +1;          // pin gpio buton  +1
  btn[2].relay_pin =rl_3 +1;  // pin gpio Relay   +1
  btn[2].channel =2;      // channel
  btn[2].ms =0;           //  if = 0 Bistable -- if > 0 Monostable for X ms
  btn[2].mem =0;
  btn[3].pin =bt_4 +1;         // pin gpio buton  +1
  btn[3].relay_pin =rl_4 +1;  // pin gpio Relay   +1 
...
void loop() {
  ...
  if (start){
    ...
    for(int i=0;i<BTN_COUNT;i++){      //  ---check relay except der have delay (staircase)
     if ( (btn[i].ms) > 0 ) {
      digitalWrite(btn[i].relay_pin-1, !LOW);        //-------------  ! for active low        
                     continue;
         } else {
       ...
       if (eep >= 2 ){
         ...
         btn[i].mem =eep;
         digitalWrite(btn[i].relay_pin-1, !eep);       //-------------  ! for active low           
