...
void ondemandwifiCallback () {
 ...
 ticker.attach(1.0, tick);
  ...
  WiFiManagerParameter custom_html_id11("<div> --- Ds18b20 1 id --- ");
  WiFiManagerParameter custom_html_id12( _ds_id1);
  WiFiManagerParameter custom_html_id13( "</div><div> --- Ds18b20 2 id --- ");
  WiFiManagerParameter custom_html_id14( _ds_id2);
  WiFiManagerParameter custom_html_id15( "</div><div> --- Ds18b20 3 id --- ");
  WiFiManagerParameter custom_html_id16( _ds_id3);
  WiFiManagerParameter custom_html_id17( "</div><div> --- Ds18b20 4 id --- ");
  WiFiManagerParameter custom_html_id18( _ds_id4);
  WiFiManagerParameter custom_html_id20( "</div>");
  WiFiManagerParameter custom_ds_id1("ds_ida", "DS Temp 1", ds_id1, 17,"required");
  WiFiManagerParameter custom_ds_id2("ds_idb", "DS Temp 2", ds_id2, 17,"required");
  WiFiManagerParameter custom_ds_id3("ds_idc", "DS Temp 3", ds_id3, 17,"required");
  WiFiManagerParameter custom_ds_id4("ds_idd", "DS Temperature", ds_id4, 17);
    ...
    if (btn[4].mem == true){  // is auto on?
     if ((Thermometer1 > 0)&& (Thermometer2 > 0)&& (Thermometer3 > 0)){  // are temps > 0ºC
      if (((Thermometer2 - Thermometer1) > Temp2) && (Thermometer1 < Temp1)){  
        if ( btn[1].ms > 10 ) {
           Serial.println("Monostable Auto on by temp for: " + String(btn[1].ms / 1000) + " seconds");
...
double get_temperature(int channelNumber, double last_val) {
   ...
   switch(channelNumber)
          {
            case 5:
         return t = sensors.getTempC(id1);     
                    break;
            case 6:
         return t = sensors.getTempC(id2);     
                    break;
            case 7:
         return t = sensors.getTempC(id3);     
                    break;       
            case 8:            
         return t = sensors.getTempC(id4);
            return t;   
                    break;                    
          }
    return t; 
           ...
           if (v==0)
             {
              ...
              if ( btn[a].ms > 10 ) {
                 if ( (btn[a].mem) == 1 ) {  
                  ...
                  Serial.println("Monostable on for: " + String(btn[a].ms / 1000) + " S");                 
                 }                                                            
              } else {
                 if ( (btn[a].mem) == 1 ) { 
