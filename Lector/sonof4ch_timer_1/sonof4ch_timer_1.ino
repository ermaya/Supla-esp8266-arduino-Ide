 ...
 */
// Defines
//#define      DefaultName       "Pool Timer"    // Default device name
...
#define      Version           "0.82 (MOD-test)"               // Firmware version
...
void loop() {
  ...
  int C_W_read2 = digitalRead(btn[3].pin-1);{  
   ...
   if (WiFi.status() == WL_CONNECTED){    
    ...
    httpUpdater.setup(server2, "/update", "admin", "pass");
...
void UpdateLED() {
  ...
  if (TimeOk == false) {
    if ( (millis() - old_ms) > 3000) {
       ...
       // digitalWrite(LED,HIGH); //1  // -------------------------- hay que revisar esto ----------------
      //  delay(50);
...
/*
    if (Output[0] == true){
       Serial.println("out main on");
        Armed = 1;
     }else{ 
        Serial.println("out main off");
        Armed = 0;
    }
    if (Output[1] == true) Serial.println("out solar on"); else Serial.println("out solar off");
    if (Output[2] == true) Serial.println("out aux 1 on"); else Serial.println("out aux 1 off");
    if (Output[3] == true) Serial.println("out aux 2 on"); else Serial.println("out aux 2 off");
    if (Output[4] == true) Serial.println("out auto/man on"); else Serial.println("out auto/man off");
*/
/*    for (byte x = 0; x < 5; x++) {
     ...
     if ((Output[x] == true) && (btn[x].mem != Output[x])){
      ...
      booton_action = true;
      CustomSupla.relayOn(btn[x].channel, btn[x].ms);
     }else if (btn[x].mem != Output[x]){
      Serial.print("set off: ");Serial.println(x);
      booton_action = true;
      CustomSupla.relayOff(btn[x].channel);
     }
    }*/
