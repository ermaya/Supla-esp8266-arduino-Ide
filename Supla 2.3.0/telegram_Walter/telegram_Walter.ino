...
#define Sensor1_PIN 14    //D5     // triger Link 1
#define Sensor2_PIN 12    //D6     // triger Link 2
#define Sensor3_PIN 13    //D7     // triger Link 3
...
class MyDigitalWrite : public Supla::Io {
  public:
    void customDigitalWrite(int channelNumber, uint8_t pin, uint8_t val) {
      ...
      }else if (channelNumber == 1) {   // ---------------------------- we select the channel we want ------------------
       if (state10 == false ){
        Serial.print("customWrite:  ");Serial.println(val); 
        alarm2_on = val; 
        EEPROM.write(296, val);
        EEPROM.commit();   
        return ::digitalWrite(pin,val);
       }
      }else if (channelNumber == 2) {   // ---------------------------- we select the channel we want ------------------
       if (state10 == false ){
        Serial.print("customWrite:  ");Serial.println(val); 
        alarm3_on = val; 
        EEPROM.write(297, val);
        EEPROM.commit();   
        return ::digitalWrite(pin,val);
       }
      }else {
    ...
    if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        if (json.success()) {Serial.println("\nparsed json");         
          ...
          strcpy(Alarm1, json["Alarm1"]);
          strcpy(Alarm2, json["Alarm2"]);
          strcpy(Alarm3, json["Alarm3"]);         
        } else {
 ...
 SuplaDevice.iterate();
   ...
   if (WiFi.status() == WL_CONNECTED){
    if (starting){
      ...
      if  (send_teleg)  {
        if ((alarm1_on) && (digitalRead(Sensor1_PIN) == 0)){
        Serial.println("send telegram");       
         url = "/bot";url += BotToken;url += "/sendMessage?chat_id=";url += Chat_Id;url += "&text=";url += Alarm1;                                                               
        Telegram_send();  
       }else if ((send_teleg)&&(!alarm1_on)){
        Serial.println("send 1 is off"); 
       }
       if ((alarm2_on) && (digitalRead(Sensor2_PIN) == 0)){
        Serial.println("send telegram");       
         url = "/bot";url += BotToken;url += "/sendMessage?chat_id=";url += Chat_Id;url += "&text=";url += Alarm2;                                                               
        Telegram_send();
       }else if ((send_teleg)&&(!alarm2_on)){
        Serial.println("send 2 is off");
       }
       if ((alarm3_on) && (digitalRead(Sensor3_PIN) == 0)){
        Serial.println("send telegram");       
         url = "/bot";url += BotToken;url += "/sendMessage?chat_id=";url += Chat_Id;url += "&text=";url += Alarm3;                                                               
        Telegram_send();
       }else if ((send_teleg)&&(!alarm3_on)){
        Serial.println("send 3 is off");     
       }
       send_teleg = false; 
       attachInterrupt(Sensor1_PIN, sense_1, FALLING);attachInterrupt(Sensor2_PIN, sense_1, FALLING);attachInterrupt(Sensor3_PIN, sense_1, FALLING);
      }
  ...
  case 17:      // -----     STATUS_REGISTERED_AND_READY
     if (state10 == true){ 
          if (EEPROM.read(295)> 0){ 
              Serial.print("mem switsh 1:");Serial.println(EEPROM.read(295));              
              alarm1_on = true;
              SuplaDevice.channelValueChanged(0,1);}
          if (EEPROM.read(296)> 0){ 
              Serial.print("mem switsh 2:");Serial.println(EEPROM.read(296));              
              alarm2_on = true;
              SuplaDevice.channelValueChanged(1,1);}
          if (EEPROM.read(297)> 0){ 
              Serial.print("mem switsh 3:");Serial.println(EEPROM.read(297));              
              alarm3_on = true;
              SuplaDevice.channelValueChanged(2,1);}
