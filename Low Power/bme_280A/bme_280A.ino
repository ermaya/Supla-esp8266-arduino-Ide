...
#define sda 4
#define scl 5
#define status_led 2    // D4  status Led   ---on when in wificonfig---blinks every SLEEP_TIME seconds in normal operation
#define sensor 12       // D6  sensor pin
#define config_pin 14   // D5  wificonfig pin  ---hold Pressed and restart to access wificonfig ---will enter wificonfig automatically on first start.
#define SLEEP_TIME 220   // sleep seconds         
int s;             
...
double get_pressure(int channelNumber, double last_val) {
      ...
      last_val = (bme.readPressure() / 100.0F);
...
void setup() {  //------------------------------------------------ Setup ----------------------------------------------
  ...
  Wire.begin(sda, scl); //(int sda, int scl) 
  bme.begin();
...
void loop() {
  ...
  if (shouldSaveConfig) {
   ...
   WiFi.begin();
    unsigned long my_start = millis();
      while ((WiFi.status() != WL_CONNECTED)  &&  (millis() - my_start < 8000)) {
            yield();
           }
     if (prim){ 
       Serial.print("connected in: ");
       Serial.println(millis() - my_start);
       prim = false;
     }
  ...
  case 17:      // -----     STATUS_REGISTERED_AND_READY
    ...
    system_deep_sleep_instant(SLEEP_TIME * 1000000);
    break;

  }
 if ((millis() - runing) > 15000){
