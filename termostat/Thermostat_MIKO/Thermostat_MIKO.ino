...
int led_pin = 13;        //D7 -- 13      
int relay_pin = 12;      //D6 -- 12  
int DS_pin = 1;          //TX --  1
int button_pin = 3;      //RX --  3 
int C_W_state = HIGH; 
 ...
 public:
  ...
  void iterateAlways() {
      ...
      // check state every x seconds
        if(relay_0->isOn()){
          double actualTemp = dsTemp->getValue();
          if(((relay_1->isOn()) && ( actualTemp >= (ThermostatTemperature + 0.5))) || ( actualTemp < -100.0)) {
            relay_1->turnOff();
          } else if ((!relay_1->isOn()) && ( actualTemp < ThermostatTemperature )){
            relay_1->turnOn();
          }
        }
      }
...
class TermostatSet : public Supla::Control::Relay {
 public:
  ...
  void turnOn(_supla_int_t duration) {
   //  increase "ThermostatTemperature" by +0.5ºC     
  ...
  void turnOff(_supla_int_t duration) {
   //  decrease "ThermostatTemperature" by -0.5ºC 
    ThermostatTemperature -= 0.5;
    state = false;
    channel.setNewValue(state);
    saveTermS = true;
...
TermostatTemp *TerT = nullptr;
TermostatSet *TerS = nullptr;
...
void ondemandwifiCallback () {
   ...
   if (!wifiManager.startConfigPortal("MIKO_Thermostat")) { Serial.println("Not connected to WiFi but continuing anyway.");} else { Serial.println("connected...yeey :)");}                
...
void setup() {
  ...
  if (LittleFS.begin()) {  // ------------------------- wificonfig read -----------------
   ...
   if((ThermostatTemperature > 99.0) || isnan(ThermostatTemperature)){
    ThermostatTemperature = 20.5;
    EEPROM.put(381, ThermostatTemperature);
    EEPROM.commit();
    Serial.println("ThermostatTemperature reset to 20.5ºC");
   }
     ...
     TerT = new TermostatTemp();
     TerS = new TermostatSet();
     TerS->getChannel()->setDefault(SUPLA_CHANNELFNC_POWERSWITCH);
     TerS->disableChannelState();
  ...
  SuplaDevice.iterate();
   ...
   if (WiFi.status() == WL_CONNECTED){
    if (starting){
      ...
      httpServer.begin(); 
      starting = false; 
      Serial.print("get DS18B20 temperature: ");
      Serial.println(dsTemp->getValue());        
     }
